...
func TestMSP04CR2AttemptReservationUsesCurrentSchemaWithoutMigration(t *testing.T) {
	if currentSchemaVersion != 3 {
		t.Fatalf("current internal schema = %d, want unchanged v3", currentSchemaVersion)
	}
	record := msp04cStoreControlFixture()
	record.attempts = nil
	first, err := encodeControlRecordV3(record)
	if err != nil {
		t.Fatalf("encode current empty-attempt record: %v", err)
	}
	second, err := encodeControlRecordV3(record)
	if err != nil {
		t.Fatalf("repeat current empty-attempt encoding: %v", err)
	}
	if !bytes.Equal(first, second) {
		t.Fatal("current empty-attempt encoding is not deterministic")
	}
	decoded, err := decodeControlRecordV3(first)
	if err != nil {
		t.Fatalf("decode current empty-attempt record: %v", err)
	}
	if len(decoded.attempts) != 0 {
		t.Fatal("current empty store invented an outgoing attempt reservation")
	}
}
...
func TestMSP04CR2AttemptValidationIsClosedBoundedAndUnique(t *testing.T) {
	tests := []struct {
		...
		{name: "duplicate attempt id", mutate: func(record *controlRecordV3) {
			record.attempts = append(record.attempts, cloneControlAttempt(record.attempts[0]))
	...
	record := msp04cr2StoreFixture()
	record.attempts = make([]controlAttempt, maxControlAttemptCount+1)
	if err := validateControlRecordV3(record); err == nil {
