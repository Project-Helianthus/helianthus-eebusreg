...
func TestMSP04CR2UnpairedPrepareRequiresActiveExactCandidateSelection(t *testing.T) {
	fixture := newMSP04CFixture(t)
	remote := msp04cSubject(901)
	wrongRemote := msp04cSubject(902)
	...
	if got := coordinator.reopen(context.Background()); got != "pairing_closed" {
		t.Fatalf("reopen unpaired fixture = %q", got)
	}
	...
	if handle, outcome := coordinator.prepareOutgoingAttempt(context.Background(), request); handle != nil || outcome != "attempt_denied" {
		t.Fatalf("unselected prepare = %q/%v, want attempt_denied/nil", outcome, handle)
	}
	coordinator.mu.Lock()
	coordinator.candidateSelectionRequired = true
	coordinator.candidateSelection = &firstTrustCandidateSelection{remote: bytes.Clone(wrongRemote)}
	coordinator.mu.Unlock()
	if handle, outcome := coordinator.prepareOutgoingAttempt(context.Background(), request); handle != nil || outcome != "attempt_denied" {
		t.Fatalf("wrong-selection prepare = %q/%v, want attempt_denied/nil", outcome, handle)
	}
	coordinator.mu.Lock()
	coordinator.candidateSelection = &firstTrustCandidateSelection{remote: bytes.Clone(remote)}
	coordinator.mu.Unlock()
	handle, outcome := coordinator.prepareOutgoingAttempt(context.Background(), request)
	if handle == nil || outcome != "attempt_reserved" {
		t.Fatalf("exact-selection prepare = %q/%v, want attempt_reserved/non-nil", outcome, handle)
	}
}

func TestMSP04CR2PersistentOperationClassCodesRemainAppendOnly(t *testing.T) {
	classes := []string{
		"first_trust",
		"revocation",
		...
		"attempt_lease_synthetic_failure",
	}
	for index, class := range classes {
		code, ok := runtimeOperationClassCode(class)
		if !ok || code != uint64(index+1) {
			t.Fatalf("operation class %q code = %d/%t, want %d/true", class, code, ok, index+1)
		}
		decoded, ok := runtimeOperationClassFromCode(code)
		if !ok || decoded != class {
			t.Fatalf("operation class code %d = %q/%t, want %q/true", code, decoded, ok, class)
		}
	}
}

func TestMSP04CR2UnpairedPrepareRejectsExpiredSelectionWithoutTimerDelivery(t *testing.T) {
	fixture := newMSP04CFixture(t)
	remote := msp04cSubject(901)
	scope := firstTrustRuntimeRetryScope(firstTrustNormalizedSKI(remote))
	fixture.store.view.control.quarantines = []firstTrustQuarantineRecord{{
	...
	coordinator.candidateSelection = &firstTrustCandidateSelection{remote: bytes.Clone(remote)}
	coordinator.mu.Unlock()
	fixture.clock.changeWall(firstTrustMaximumWindow + time.Second)
	...
	if handle != nil || outcome != "attempt_denied" {
		t.Fatalf("expired selected prepare = %q/%v, want attempt_denied/nil", outcome, handle)
	}
}
...
func TestMSP04CR2AbortPreparedChargesSyntheticFailureExactlyOnce(t *testing.T) {
	fixture, coordinator, remote, scope := newMSP04CR2AttemptFixture(t)
	handle, outcome := coordinator.prepareOutgoingAttempt(
		context.Background(),
		msp04cr2Request(remote, "peer.invalid", 4712, "/ship/"),
	...
	if got := coordinator.abortPreparedOutgoingAttempt(context.Background(), handle); got != "attempt_aborted" {
		t.Fatalf("abort = %q, want attempt_aborted", got)
	}
	if fixture.store.prepared.operationClass != "attempt_abort_synthetic_failure" {
		t.Fatalf("abort operation class = %q", fixture.store.prepared.operationClass)
	}
	if _, ok := soleMSP04CR2Attempt(coordinator); ok {
		t.Fatal("abort retained the reservation")
	}
	state, count, remaining, ok := coordinator.retryState(scope)
	if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
		t.Fatalf("abort retry tuple = %s/%d/%s/%t, want BACKOFF_ACTIVE/1/3s/true", state, count, remaining, ok)
	}

	if got := coordinator.abortPreparedOutgoingAttempt(context.Background(), handle); got != "stale_attempt" {
		t.Fatalf("repeated abort = %q, want stale_attempt", got)
	}
	state, count, remaining, ok = coordinator.retryState(scope)
	if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
		t.Fatalf("repeated abort charged twice: %s/%d/%s/%t", state, count, remaining, ok)
	}
}

func TestMSP04CR2ReservedAttemptLeaseExpiresOnInjectedMonotonicClock(t *testing.T) {
	fixture, coordinator, remote, scope := newMSP04CR2AttemptFixture(t)
	scheduler := &msp04cr2Scheduler{now: fixture.clock.MonotonicNow}
	coordinator.outgoingAttemptLease = 5 * time.Second
	...
	scheduler.runDue()
	if _, ok := soleMSP04CR2Attempt(coordinator); !ok {
		t.Fatal("lease expired before its monotonic deadline")
	}
	fixture.clock.advanceMonotonic(5 * time.Second)
	scheduler.runDue()
	if _, ok := soleMSP04CR2Attempt(coordinator); ok {
		t.Fatal("lease expiry retained the reservation")
	}
	state, count, remaining, ok := coordinator.retryState(scope)
	if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
		t.Fatalf("lease expiry retry tuple = %s/%d/%s/%t", state, count, remaining, ok)
	}
	if fixture.store.prepared.operationClass != "attempt_lease_synthetic_failure" {
		t.Fatalf("lease operation class = %q", fixture.store.prepared.operationClass)
	}

	scheduler.runDue()
	state, count, remaining, ok = coordinator.retryState(scope)
	if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
		t.Fatalf("lease expiry charged twice: %s/%d/%s/%t", state, count, remaining, ok)
	}
}

func TestMSP04CR2LeaseExpirySerializesWithConcurrentAuthorization(t *testing.T) {
	fixture, coordinator, remote, scope := newMSP04CR2AttemptFixture(t)
	scheduler := &msp04cr2Scheduler{now: fixture.clock.MonotonicNow}
	coordinator.outgoingAttemptLease = 5 * time.Second
	coordinator.outgoingAttemptSchedule = scheduler.afterFunc
	...
	authorized := make(chan firstTrustOutgoingAttemptPermit, 1)
	...
	if _, ok := soleMSP04CR2Attempt(coordinator); ok {
		t.Fatal("authorization/expiry race retained an active attempt")
	}
	state, count, remaining, ok := coordinator.retryState(scope)
	if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
		t.Fatalf("authorization/expiry retry tuple = %s/%d/%s/%t", state, count, remaining, ok)
	}
	if permit.decision != "DENY" && permit.decision != "PERMIT" {
		t.Fatalf("authorization/expiry race returned malformed permit: %#v", permit)
	}
}

func TestMSP04CR2AuthorizeRejectsExpiredLeaseWithoutTimerDelivery(t *testing.T) {
	fixture, coordinator, remote, scope := newMSP04CR2AttemptFixture(t)
	scheduler := &msp04cr2Scheduler{now: fixture.clock.MonotonicNow}
	coordinator.outgoingAttemptLease = 5 * time.Second
	coordinator.outgoingAttemptSchedule = scheduler.afterFunc
	...
	if outcome != "attempt_denied" || permit.decision != "DENY" || permit.reason != "POLICY_DENIED" {
		t.Fatalf("expired lease authorization = %q/%#v", outcome, permit)
	}
	if _, ok := soleMSP04CR2Attempt(coordinator); ok {
		t.Fatal("expired lease authorization retained an active attempt")
	}
	state, count, remaining, ok := coordinator.retryState(scope)
	if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
		t.Fatalf("expired lease retry tuple = %s/%d/%s/%t", state, count, remaining, ok)
	}
	scheduler.runDue()
	state, count, remaining, ok = coordinator.retryState(scope)
	if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
		t.Fatalf("delayed lease callback charged twice: %s/%d/%s/%t", state, count, remaining, ok)
	}
}

func TestMSP04CR2AuthorizeRechecksSelectionAfterDurableCommit(t *testing.T) {
	fixture, coordinator, remote, scope := newMSP04CR2UnpairedSelectedFixture(t)
	handle, outcome := coordinator.prepareOutgoingAttempt(
		context.Background(),
		msp04cr2Request(remote, "peer.invalid", 4712, "/ship/"),
	)
	...
	fixture.store.block()
	result := make(chan firstTrustOutgoingAttemptPermit, 1)
	go func() {
		permit, _ := coordinator.authorizeOutgoingAttempt(context.Background(), handle)
		result <- permit
	}()
	...
	select {
	case permit := <-result:
		if permit.decision != "DENY" || permit.reason != "POLICY_DENIED" {
			t.Fatalf("post-deadline authorization = %#v, want policy denial", permit)
		}
	case <-time.After(time.Second):
		t.Fatal("post-deadline authorization did not terminate")
	}
	if _, ok := soleMSP04CR2Attempt(coordinator); ok {
		t.Fatal("post-deadline authorization retained an active attempt")
	}
	state, count, remaining, ok := coordinator.retryState(scope)
	if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
		t.Fatalf("post-deadline retry tuple = %s/%d/%s/%t", state, count, remaining, ok)
	}
}

func TestMSP04CR2ShutdownSettlesEveryActiveAttemptBeforeCoordinatorClose(t *testing.T) {
	for _, authorize := range []bool{false, true} {
		t.Run(map[bool]string{false: "reserved", true: "authorized"}[authorize], func(t *testing.T) {
			...
			if authorize {
				...
				if result != "attempt_permitted" || permit.decision != "PERMIT" {
					t.Fatalf("authorize = %q/%#v", result, permit)
				}
			}
			...
			if _, ok := soleMSP04CR2Attempt(coordinator); ok {
				t.Fatal("bridge shutdown retained an active attempt")
			}
			state, count, remaining, ok := coordinator.retryState(scope)
			if !ok || state != "BACKOFF_ACTIVE" || count != 1 || remaining != 3*time.Second {
				t.Fatalf("shutdown retry tuple = %s/%d/%s/%t", state, count, remaining, ok)
			}
			select {
			case <-handle.context.Done():
			default:
				t.Fatal("bridge shutdown did not cancel the attempt context")
			}
		})
	}
}
...
func newMSP04CR2UnpairedSelectedFixture(t *testing.T) (*msp04cFixture, *firstTrustCoordinator, []byte, [32]byte) {
	t.Helper()
	fixture := newMSP04CFixture(t)
	remote := msp04cSubject(901)
	scope := firstTrustRuntimeRetryScope(firstTrustNormalizedSKI(remote))
...
type msp04cr2Scheduler struct {
	mu    sync.Mutex
	now   func() time.Duration
	tasks []*msp04cr2ScheduledTask
}

type msp04cr2ScheduledTask struct {
	mu       sync.Mutex
...
func (scheduler *msp04cr2Scheduler) afterFunc(delay time.Duration, callback func()) firstTrustOutgoingAttemptTimer {
	task := &msp04cr2ScheduledTask{
		deadline: firstTrustSaturatingDurationAdd(scheduler.now(), delay),
		callback: callback,
	}
	scheduler.mu.Lock()
...
func (scheduler *msp04cr2Scheduler) runDue() {
	scheduler.mu.Lock()
	tasks := append([]*msp04cr2ScheduledTask(nil), scheduler.tasks...)
	scheduler.mu.Unlock()
	for _, task := range tasks {
		task.mu.Lock()
		if task.stopped || task.fired || scheduler.now() < task.deadline {
			task.mu.Unlock()
			continue
		}
		task.fired = true
...
func (task *msp04cr2ScheduledTask) Stop() bool {
	task.mu.Lock()
